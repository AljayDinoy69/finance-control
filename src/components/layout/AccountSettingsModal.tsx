...
export default function AccountSettingsModal({ open, onOpenChange }: Props) {
  ...
  const save = () => { setProfile({ name: draft.name.trim() || 'User', email: draft.email.trim() || 'user@example.com' }); setPreferences({ currency: draft.currency, compactMode: draft.compactMode }); setTheme(draft.theme); onOpenChange(false) }
